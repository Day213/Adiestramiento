...
$pagina_actual = isset($_GET['solicitudes-respondidas']) ? (int)$_GET['solicitudes-respondidas'] : 1;
$inicio = ($pagina_actual - 1) * $registros_por_pagina;

$total_sql = "SELECT COUNT(*) as total FROM gestion WHERE status = 0";
$total_result = $conexion->query($total_sql);
$total_fila = $total_result->fetch_assoc();
$total_registros = $total_fila['total'];
$total_paginas = ceil($total_registros / $registros_por_pagina);

$sql = "SELECT * FROM gestion WHERE status = 0 LIMIT $inicio, $registros_por_pagina";
...
<div class="flex justify-center mt-4">
  <nav class="inline-flex shadow-sm rounded-md" aria-label="Pagination">
    <?php if ($pagina_actual > 1): ?>
      <a href="?solicitudes-respondidas=<?php echo $pagina_actual - 1; ?>"
        class="bg-white hover:bg-gray-100 p-2 border border-gray-300 text-gray-500">Anterior</a>
    <?php endif; ?>
    <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
      <a href="?solicitudes-respondidas=<?php echo $i; ?>"
