...
?>
  <div class="flex justify-center items-center gap-2 py-4">
    ...
    for ($i = 1; $i <= $total_paginas; $i++) {
      ...
      echo "<a class='" . $clase_activa . "' href='?pagina_respondidas=" . $i . "'>" . $i . "</a>";
    }
    if ($pagina_actual < $total_paginas) {
      echo "<a class='bg-slate-300 p-2 rounded-md text-xs' href='?pagina_respondidas=" . ($pagina_actual + 1) . "'>Siguiente &raquo;</a>";
    }
    ?>
  </div>
<?php }
